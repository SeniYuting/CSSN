...
public class JWStatisticsPanel extends JPanel {
	...
	public void initPanel(){
		...
		// -----------------------课程统计----------------------
		final JButton jp3jb13 = new JButton("选择课程");
		jp3jb13.setBounds(250, 290, 100, 35);
		jp3jb13.setFont(new Font("楷体", Font.PLAIN, 18));
		
		//"课程号","课程名"
		String[] jp3title4 = {"课程号","课程名"};  //得选课学生数
		...
		jp3jb13.addActionListener(new ActionListener() { //选择课程

			public void actionPerformed(ActionEvent e) {
				int row = jp3table4.getSelectedRow();
				if(row>=0){
					RMIFactory factory = Client.getFactory();
					try {
						StatisticsBLService stqservice = factory.getSTQService();
						String courseNo = (String)jp3table4.getValueAt(row, 0);
						StatisticsVO stVO = stqservice.checkCourseStatistics(courseNo);
						
						int stuNo = stVO.getNum();
